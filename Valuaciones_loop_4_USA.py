...
def process_ticket(ticket, ruta):
    print(f"Procesando {ticket}...")
    ...
    try:
        ...
        # Check for multiple possible fields to determine valid columns
        fields_to_check = ["Revenue per Share", "Earnings per Share (Diluted)", "Dividends per Share"]
        row = None
        for field in fields_to_check:
            matching_rows = df_hist.loc[df_hist["Fiscal Period"].eq(field)]
            if not matching_rows.empty:
                row = matching_rows.iloc[0]
                break
        
        if row is None:
            # Fallback to checking any row if the preferred fields are missing
            row = df_hist.iloc[0]
        ...
        indice_ttm = df_err.columns.tolist().index(nombre_ttm)  
        ...
        def agregar_razon_financiera(df_anual_filtrado, fila_numerador, nombre_nueva_fila,
                                     *filas_denominadoras, como_porcentaje=True,
                                     calculo="", explicacion=""):
            nonlocal razones_financieras

            df_num = df_anual_filtrado[df_anual_filtrado['Fiscal Period'] == fila_numerador]
            if df_num.empty:
                return
            fila_num = df_num.iloc[0, 1:]

            denominadores_validos = []
            for fila in filas_denominadoras:
                df_den = df_anual_filtrado[df_anual_filtrado['Fiscal Period'] == fila]
                if not df_den.empty:
                    denominadores_validos.append(df_den.iloc[0, 1:].values)
                else:
                    return

            fila_den = sum(denominadores_validos)
        ...
        def agregar_razon_financiera_numerador(df_anual_filtrado, nombre_nueva_fila,
            ...
            df_num1 = df_anual_filtrado[df_anual_filtrado['Fiscal Period'] == fila_numerador_1]
            df_num2 = df_anual_filtrado[df_anual_filtrado['Fiscal Period'] == fila_numerador_2]
            df_den = df_anual_filtrado[df_anual_filtrado['Fiscal Period'] == fila_denominador]

            if df_num1.empty or df_num2.empty or df_den.empty:
                return

            fila_num_1 = df_num1.iloc[0, 1:]
            fila_num_2 = df_num2.iloc[0, 1:]
            fila_den = df_den.iloc[0, 1:]
        ...
        def agregar_fila(df_anual_filtrado, fila_nombre, nombre_nueva_fila,
            ...
            df_fila = df_anual_filtrado[df_anual_filtrado['Fiscal Period'] == fila_nombre]
            if df_fila.empty:
                return
            fila = df_fila.iloc[0, 1:]
        ...
        def dividir_filas_razones_financieras(fila_numerador, fila_denominador,
            ...
            if fila_numerador not in razones_financieras.index or fila_denominador not in razones_financieras.index:
                return
        ...
        ratios_existentes = [r for r in ratios_absolutos if r in razones_financieras.index]

        if ratios_existentes:
            razones_financieras.loc[ratios_existentes, razones_financieras.columns[2:]] = (
                razones_financieras.loc[ratios_existentes, razones_financieras.columns[2:]]
                .astype(float)
                .abs()
            )
        ...
        if "Operating Margin(%)" in razones_financieras.index:
            margen_operativo = (
                razones_financieras
            ...
            margen_operativo = pd.Series(0, index=df_anual_filtrado.columns[1:])
        ...
        if "Sales" in razones_financieras.index:
            ventas = (
                razones_financieras
                    .loc["Sales"]
                    .drop(["Calculo", "Explicacion"])
            ...
            ventas = pd.Series(0, index=df_anual_filtrado.columns[1:])
        ...
        re_cost  = df_wacc["Cost_of_Equity"]
        rd  = df_wacc["Cost_of_Debt"]
        tax = df_wacc["Tax_Rate"]

        df_wacc["WACC"] = ((E / (E + D)) * re_cost + (D / (E + D)) * rd * (1 - tax)).round(5)
        ...
        anio_inicio = mediana_growth_10y.index[0]  
        ...
        def calcular_cagr_por_año_10(df, columna, ventana_max=10, min_anios=2):
            df = df.copy()
            df["Fecha_Temp"] = pd.to_datetime(df["Año"], format="%b%Y") + pd.offsets.MonthEnd(0)
            df = df.sort_values("Fecha_Temp").reset_index(drop=True)
            resultados = []
        ...
        df_cagr_fcf_10 = calcular_cagr_por_año_10(df_fcf, "Free_Cash_Flow")


        if len(df_fcf_crecimiento_10.columns) > 1:
            col = df_fcf_crecimiento_10.columns[1]
            df_fcf_crecimiento_10[col] = df_fcf_crecimiento_10[col].clip(lower=0.05, upper=0.25)

        if len(df_cagr_fcf_10.columns) > 1:
        ...
        x_fcf_avg  = calcular_x(df_fcf_crecimiento_10, discount_rate) if not df_fcf_crecimiento_10.empty else pd.DataFrame(columns=["Año", "x"])
        x_cagr_fcf = calcular_x(df_cagr_fcf_10, discount_rate) if not df_cagr_fcf_10.empty else pd.DataFrame(columns=["Año", "x"])
        ...
        def calcular_intrinsic_value_fcf(df_x, df_y, df_fcf_per_share):
            if df_x.empty:
                return pd.DataFrame(columns=["Intrinsic_Value_FCF"])
        ...
        df_cagr_eps_10 = calcular_cagr_por_año_10(df_eps, "EPS")


        if len(df_eps_crecimiento_10.columns) > 1:
            col = df_eps_crecimiento_10.columns[1]
            df_eps_crecimiento_10[col] = df_eps_crecimiento_10[col].clip(lower=0.05, upper=0.20)

        if len(df_cagr_eps_10.columns) > 1:
        ...
        df_eps_crecimiento_10_x = calcular_x(df_eps_crecimiento_10, discount_rate) if not df_eps_crecimiento_10.empty else pd.DataFrame(columns=["Año", "x"])
        df_cagr_eps_10_x = calcular_x(df_cagr_eps_10, discount_rate) if not df_cagr_eps_10.empty else pd.DataFrame(columns=["Año", "x"])
        ...
        def calcular_intrinsic_value_eps(df_x, df_y, df_eps):
            if df_x.empty:
                return pd.DataFrame(columns=["Intrinsic_Value_eps"])
        ...
        if len(df_mediana_growth_10y_eps_value.columns) > 1:
        ...
        if len(df_eps_crecimiento_10_value.columns) > 1:
        ...
        if len(df_cagr_eps_10_value.columns) > 1:
        ...
        headers_df["Mes"] = headers_df["Header"].str[:3].map(mes_map)
        headers_df["Año"] = headers_df["Header"]  
        ...
        if pd.notna(anio_max):
            ...
            metodos_top = pd.Series(dtype=str)
        ...
        os.makedirs("Valuaciones_USA", exist_ok=True)
        ...
        print(f"Ticket {ticket} procesado exitosamente.")

    except Exception as e:
        print(f"Error procesando el ticket {ticket}: {e}")

def procesar_todos_los_tickets(ruta_usa="USA/"):
    tickets = [f.replace('.xlsx', '') for f in os.listdir(ruta_usa) if f.endswith('.xlsx')]
    print(f"Se encontraron {len(tickets)} tickets a procesar.")
    for ticket in tickets:
        process_ticket(ticket, ruta_usa)
    print("Se terminó de procesar todos los tickets.")
