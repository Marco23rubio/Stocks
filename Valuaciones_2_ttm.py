...
for archivo_excel in archivos_usa:
    ...
    try:
        ...
        archivo_stock = f'Stocks/USA/DatosHistoricos/Datos históricos {ticket}'
        
        if not os.path.exists(f'{archivo_stock}.csv'):
            print(f'  -> No se encontró {archivo_stock}.csv. Saltando ticket...')
            continue
        archivo_market = "Stocks/Datos históricos del S&P 500 TR"
        market_risk_premium = 0.07
        archivo_market_bonos = "Stocks/bonos_usa"
        archivo_inflacion_usa = "Stocks/inflation_usa"
        ...
        r_e = df_wacc["Cost_of_Equity"]
        r_d = df_wacc["Cost_of_Debt"]
        tax = df_wacc["Tax_Rate"]

        df_wacc["WACC"] = ((E / (E + D)) * r_e + (D / (E + D)) * r_d * (1 - tax)).round(5)
